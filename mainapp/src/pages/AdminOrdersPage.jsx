...
export default function AdminOrdersPage() {
  ...
  const handleDownloadPDF = async (orderId, index, filename, fallbackUrl) => {
    try {
      if (!orderId) {
        toast.error('Order ID is missing');
        return;
      }
      const res = await api.get(`/api/orders/${orderId}/printout/${index}/download`, {
        responseType: 'blob'
      });
      ...
      console.error('Proxy download failed, trying direct fallback:', err);
      if (!fallbackUrl) {
      ...
      try {
        ...
        document.body.removeChild(link);
        window.URL.revokeObjectURL(blobUrl);
        toast.success('Download completed via fallback');
      } catch (fallbackErr) {
                                  ...
                                  {item.pdfUrl ? (
                                    <div className="space-y-1">
                                      ...
                                      <div className="pt-2 flex items-center justify-between">
                                        ...
                                        <button
                                          type="button"
                                          onClick={() => handleDownloadPDF(order._id, idx, item.pdfName || 'document.pdf', item.pdfUrl)}
                        ...
                        <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                          ...
                          {order.orderStatus === 'Confirmed' && order.products?.some(p => p.type === 'printout') && (
                            <button
                              ...
                              className="bg-indigo-600 text-white font-extrabold text-xs px-3.5 py-2 rounded-xl shadow-xs active:scale-95 transition-all"
                            >
                              Mark Printing
                            </button>
                          )}
                          ...
                          {['Order Placed', 'Confirmed', 'Printing'].includes(order.orderStatus) && (
                            <button
                              ...
                              className="bg-[#40A2E3] text-white font-extrabold text-xs px-3.5 py-2 rounded-xl shadow-xs active:scale-95 transition-all"
                            >
                              Mark Packing
                            </button>
                          )}

                          {['Preparing', 'Packing', 'Printing'].includes(order.orderStatus) && (
                            <button
