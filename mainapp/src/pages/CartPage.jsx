...
export default function CartPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 pb-24">
      ...
      {!navigator.onLine && (
        <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-3.5 flex items-center space-x-3 text-xs text-[#0F172A] dark:text-slate-200 animate-pulse">
          <AlertCircle size={20} className="text-red-500 flex-shrink-0" />
          <div>
            <p className="font-extrabold text-red-500">Checkout is Unavailable</p>
            <p className="text-[11px] text-[#64748B] dark:text-slate-400">Please connect to the internet to complete your order.</p>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/95 dark:bg-[#0F172A]/95 border-t border-[#E2E8F0] dark:border-[#334155] backdrop-blur-md z-40">
        <div className="max-w-2xl mx-auto flex items-center justify-between gap-4">
          ...
          <button
            onClick={() => navigator.onLine && navigate('/checkout')}
            disabled={!navigator.onLine}
            className={`flex-1 font-black text-xs py-3.5 px-6 rounded-2xl flex items-center justify-center space-x-2 transition-all ${
              navigator.onLine 
                ? 'bg-[#40A2E3] hover:bg-[#40A2E3]/90 text-white shadow-lg shadow-[#40A2E3]/25 active:scale-[0.99]' 
                : 'bg-slate-200 dark:bg-slate-800 text-slate-450 dark:text-slate-500 cursor-not-allowed border border-sys-border'
            }`}
          >
            <span>{navigator.onLine ? 'Proceed to Checkout' : 'Offline • Reconnect to Order'}</span>
            {navigator.onLine && <ArrowRight size={16} />}
          </button>
        </div>
      </div>
