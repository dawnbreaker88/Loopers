...
export default function ProductResultsPage() {
  ...
  const handleCategorySelect = (catName) => {
    if (catName && (catName.toLowerCase().trim() === 'printouts' || catName.toLowerCase().trim() === 'printout' || catName.toLowerCase().trim().includes('print'))) {
      navigate('/printouts');
      return;
    }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-9.5rem)] sm:h-[calc(100vh-7rem)] max-w-7xl mx-auto overflow-hidden space-y-3">
      ...
      <div className="flex-1 flex gap-2 sm:gap-4 min-h-0 overflow-hidden">
        ...
        <section className="flex-1 h-full overflow-y-auto pl-0.5 pr-1 pb-4 scrollbar-hide">
          {loading ? (
            ...
            <OfflineView onRetry={() => dispatch(fetchProducts({ category: selectedCategory === 'All' ? '' : selectedCategory, search: searchQuery }))} />
          ) : (
