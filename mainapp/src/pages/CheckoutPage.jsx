  ...
  const executeFinalOrder = async () => {
    ...
    try {
      ...
      if (response.success && response.order) {
        ...
        updatePWAEngagement('place_order');
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 pb-28">
      ...
      <div className="bg-sys-surface border border-sys-border rounded-2xl p-4 space-y-3 shadow-xs">
        ...
        {/* UPI QR & Mobile App Launch Display */}
        {paymentMethod === 'UPI' && (() => {
          const upiId = import.meta.env.VITE_UPI_ID || '9391472578@ybl';
          const upiName = import.meta.env.VITE_UPI_NAME || 'Loopers Quick Commerce';
          const upiIntentUri = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(upiName)}&am=${finalTotal.toFixed(2)}&cu=INR&tn=${encodeURIComponent('Loopers Order Payment')}`;
          const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(upiIntentUri)}`;

          return (
            <div className="mt-3 bg-slate-50 dark:bg-slate-800/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 flex flex-col items-center text-center space-y-3 animate-fade-in">
              ...
              <p className="text-[11px] font-bold text-[#0F172A] dark:text-white">
                Scan QR or tap below to pay <span className="text-[#40A2E3] font-mono">₹{finalTotal.toFixed(2)}</span>
              </p>

              {/* Mobile UPI App Launcher Intent Button */}
              <a
                href={upiIntentUri}
                className="w-full bg-[#40A2E3] hover:bg-[#40A2E3]/90 text-white font-extrabold text-xs py-2.5 px-4 rounded-xl shadow-xs flex items-center justify-center space-x-2 active:scale-95 transition-all cursor-pointer"
              >
              ...
              <div className="flex items-center space-x-1.5 bg-white dark:bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-mono font-bold text-[#64748B] dark:text-slate-300">
                ...
                <button
                  ...
                  className="text-[#40A2E3] hover:text-[#40A2E3]/80"
                >
                  <Copy size={13} />
                </button>
              </div>
