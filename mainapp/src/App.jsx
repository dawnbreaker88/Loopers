...
import { loadUserProfile, logout } from './store/authSlice.js';
...
// Page Imports (Lazy Loaded for modular high performance bundle splitting)
const LandingPage = lazy(() => import('./pages/LandingPage.jsx'));
const LoginPage = lazy(() => import('./pages/LoginPage.jsx'));
const SignupPage = lazy(() => import('./pages/SignupPage.jsx'));
const DashboardPage = lazy(() => import('./pages/DashboardPage.jsx'));
const ProductResultsPage = lazy(() => import('./pages/ProductResultsPage.jsx'));
...
const PrintoutsPage = lazy(() => import('./pages/PrintoutsPage.jsx'));
import useGeoLocation from './hooks/useGeoLocation.js';

import OfflineBanner from './components/OfflineBanner.jsx';
import PWAInstallModal from './components/PWAInstallModal.jsx';
import PWAWelcomeModal from './components/PWAWelcomeModal.jsx';
...
function AppContent() {
    ...
    const handleStorageChange = (e) => {
      if (e.key === 'token' || e.key === 'user') {
        ...
        if (!storedToken || !storedUser) {
          ...
          dispatch(loadUserProfile(storedToken));
        }
      }
    };
  ...
  "
      >
        <Navbar />
        <main className="flex-grow container mx-auto px-4 pt-safe pb-[calc(5.5rem+env(safe-area-inset-bottom))] md:pb-8 max-w-7xl">
          <NotificationPermissionPrompt />
          <Suspense fallback={
            <div className="flex items-center justify-center min-h-[50vh] w-full">
              <BrandedLoader message="Loading page..." />
            </div>
          }>
            {isStoreClosed ? (
              <Routes>
