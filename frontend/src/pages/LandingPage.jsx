...
export default function LandingPage() {
  ...
  const handleBannerClick = (b) => {
    ...
    if (type === 'category' && target) {
      if (target.toLowerCase() === 'printouts') {
        navigate('/app/printouts');
      } else {
        navigate(`/app/products?category=${encodeURIComponent(target)}`);
      }
    } else if (type === 'product' && target) {
      navigate(`/app/products?search=${encodeURIComponent(target)}`);
  ...
  return (
    <div className="space-y-4 pb-20">
              ...
              <div
                ...
                onClick={() => {
                  if (cat.name === 'Printouts') {
                    ...
                    navigate(`/app/products?category=${encodeURIComponent(cat.name)}`);
                  }
                }}
      ...
      {/* 5b. Premium Instant Document Printing Service Banner */}
      <div 
        onClick={() => navigate('/app/printouts')}
        className="relative overflow-hidden rounded-3xl border border-sys-border bg-gradient-to-r from-[#40A2E3]/20 via-[#40A2E3]/5 to-[#40A2E3]/10 p-5 flex flex-col sm:flex-row items-center justify-between gap-4 cursor-pointer hover:scale-[1.01] active:scale-95 transition-all shadow-xs group"
      >
        <div className="space-y-1.5 text-center sm:text-left">
          <span className="inline-flex items-center space-x-1 text-[9px] font-black uppercase tracking-wider text-[#40A2E3] bg-[#40A2E3]/10 px-2.5 py-0.5 rounded-full">
            <Sparkles size={10} className="fill-current mr-0.5 animate-pulse text-[#40A2E3]" />
            <span>New Service</span>
          </span>
          <h4 className="text-sm font-black text-[#0F172A] dark:text-white">
            Instant Document Printing & Delivery
          </h4>
          <p className="text-xs text-[#64748B] dark:text-slate-400 max-w-md">
            Upload PDF, Word, Slides, or Images. Customize sizes, binding, copies, and get them delivered to your door in 15 mins!
          </p>
        </div>
        <button 
          type="button"
          className="shrink-0 bg-[#40A2E3] hover:bg-[#40A2E3]/90 text-white text-xs font-black px-5 py-3 rounded-2xl shadow-md shadow-[#40A2E3]/20 transition-all flex items-center space-x-1.5 group-hover:scale-[1.03]"
        >
