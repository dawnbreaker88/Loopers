      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4 max-w-md mx-auto px-4">
        ...
        <div>
          ...
          <p className="text-xs text-[#64748B] dark:text-slate-400 mt-1">
            Looks like you haven't added snacks or essentials yet!
          </p>
        </div>
        <button
    ...
    <div className="max-w-2xl mx-auto space-y-4 pb-24">
      ...
      <div className="bg-sys-surface border border-sys-border rounded-2xl p-4 space-y-4 shadow-xs">
        {items.map((item) => {
          ...
          const detailsString = isPrintout 
            ? `${item.paperSize} • ${item.printMode === 'double' ? 'Double Side' : 'Single Side'} • ${item.colorPages > 0 ? 'Color' : 'B&W'} • ${item.pages} pgs • ${item.copies} cps${item.binding && item.binding !== 'none' ? ` • ${item.binding} binding` : ''}${item.extras?.includes('lamination') ? ' • Laminated' : ''}`
            : `${product.unit || 'unit'}`;

          const unitPrice = isPrintout
            ? item.price
            : (product.discount ? product.price - (product.price * product.discount) / 100 : product.price);

          const itemTotal = unitPrice * item.quantity;
          const itemId = isPrintout ? item._id : product._id;
          ...
          return (
            ...
            >
              {/* Thumbnail */}
              <div className="w-14 h-14 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700/50 flex-shrink-0 overflow-hidden flex items-center justify-center">
                {isPrintout ? (
                  <FileText size={24} className="text-[#40A2E3]" />
                ) : (
