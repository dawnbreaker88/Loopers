                    ...
                    <div key={idx} className="flex flex-col text-xs text-[#0F172A] dark:text-slate-200">
                      <div className="flex justify-between">
                        <span className="truncate max-w-[75%] font-medium">
                          {item.quantity}x {item.name || (isPrintout ? 'Printout Document' : 'Item')}
                        </span>
                      ...
                      {isPrintout && (
                        <span className="text-[10px] text-[#64748B] dark:text-slate-400 mt-0.5 ml-3 font-medium">
                          Specs: {item.paperSize} • {item.printMode === 'double' ? 'Double Side' : 'Single Side'} • {item.colorPages > 0 ? 'Color' : 'B&W'} • {item.pages} pgs • {item.copies} cps{item.binding && item.binding !== 'none' ? ` • ${item.binding} binding` : ''}{item.extras?.includes('lamination') ? ' • Laminated' : ''}
                        </span>
                      )}
                    </div>
                  );
