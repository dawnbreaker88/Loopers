...
export const initSocket = (user, token) => {
  ...
  socket.on('connect', () => {
    if (import.meta.env.DEV) console.log('[SocketService] Connected:', socket.id);
  ...
  socket.on('new-order', (order) => {
    if (import.meta.env.DEV) console.log('[SocketService] Received new-order:', order?._id);
  ...
  socket.on('orderUpdated', (order) => {
    if (import.meta.env.DEV) console.log('[SocketService] Received orderUpdated:', order?._id);
    store.dispatch(orderUpdatedSocket(order));
    listeners.forEach(cb => cb('orderUpdated', order));
  });

  const handleRiderLocation = (data) => {
    if (import.meta.env.DEV) console.log('[SocketService] Received riderLocationUpdate:', data);
  ...
  socket.on('productCreated', (product) => {
    if (import.meta.env.DEV) console.log('[SocketService] Received productCreated:', product?._id);
    store.dispatch(productAddedFromSocket(product));
    listeners.forEach(cb => cb('productCreated', product));
  });

  socket.on('productUpdated', (product) => {
    if (import.meta.env.DEV) console.log('[SocketService] Received productUpdated:', product?._id);
    store.dispatch(productUpdatedFromSocket(product));
    listeners.forEach(cb => cb('productUpdated', product));
  });

  socket.on('productDeleted', (data) => {
    if (import.meta.env.DEV) console.log('[SocketService] Received productDeleted:', data);
  ...
  socket.on('storeStatusUpdated', (storeData) => {
    if (import.meta.env.DEV) console.log('[SocketService] Received storeStatusUpdated:', storeData);
  ...
  const forwardStatusEvent = (eventName) => {
    socket.on(eventName, (data) => {
      if (import.meta.env.DEV) console.log(`[SocketService] Received ${eventName}:`, data);
  ...
  socket.on('disconnect', (reason) => {
    if (import.meta.env.DEV) console.log('[SocketService] Disconnected:', reason);
  });
