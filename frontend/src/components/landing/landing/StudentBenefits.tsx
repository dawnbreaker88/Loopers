...
const benefits = [
  { icon: GraduationCap, title: "Student-Friendly Experience", tint: "bg-primary-soft text-primary" },
    ...
    <section aria-labelledby="benefits-heading" className="relative py-20 lg:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8">
        <SectionHeading
          ...
          title={<>Built around <em className="text-secondary not-italic">student life.</em></>}
        />
        ...
        <div className="mt-14 grid grid-cols-2 md:grid-cols-3 gap-4">
          {benefits.map((b, i) => (
            ...
            >
              <div className={`grid place-items-center h-12 w-12 shrink-0 rounded-2xl ${b.tint} transition-transform group-hover:scale-110`}>
                <b.icon className="h-6 w-6" strokeWidth={2} />
              </div>
              <span className="text-sm sm:text-base font-semibold text-foreground leading-tight">{b.title}</span>
            </motion.div>
          ))}
        </div>
      </div>
